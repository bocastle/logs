...
외부 API는 내가 통제할 수 없는 경계다. 응답이 늦어지거나 실패했을 때 아무 기준 없이 기다리면, 장애는 외부 서비스에서 끝나지 않고 내 서버의 커넥션, 스레드, 큐까지 붙잡는다.

그래서 외부 연동은 성공 경로보다 실패 경로를 먼저 정해두는 편이 안전하다. 이 글은 동기 호출을 기준으로 타임아웃, 자원 격리, 서킷 브레이커, 재시도, 폴백을 어떻게 묶어 보는지 정리한 기록이다.

---

## 1) 먼저 잡는 기준: 빠른 실패와 자원 격리
...
> 타임아웃은 **지수 백오프+재시도**와 함께 계산해야 한다. 개별 호출 시간은 짧아 보여도, 여러 번 재시도하면 사용자가 체감하는 지연은 쉽게 예산을 넘는다.
...
> 임계값은 **요청 수 최소 기준**(sliding window)과 함께 설정하고, **Slow Call Rate**(예: > 1s)를 별도로 감지한다.
...
- **타임아웃 + 벌크헤드 + 서킷 브레이커**는 동기 외부 호출을 보호하는 기본 축이다.
- 여기에 **재시도(백오프+지터)**, **Rate Limit/Load Shedding**, **캐시/폴백**을 더해 빠른 실패와 품질 저하 모드를 설계한다.
- 운영 단계에서는 지표와 추적 로그를 보면서 기준을 조정하고, 장애 주입 테스트로 방어 장치가 실제로 동작하는지 확인해야 한다.
