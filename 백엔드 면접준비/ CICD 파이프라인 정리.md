...
배포 흐름을 정리할 때는 "어떤 도구를 쓰는가"보다 변경이 안전하게 서비스까지 도착하는지가 먼저 보인다. 커밋이 들어온 뒤 테스트, 빌드, 패키징, 배포, 검증이 같은 순서로 반복되어야 장애가 났을 때도 원인을 좁히기 쉽다.

이 글은 CI/CD를 용어로만 외우기보다, 실제 운영에서 어떤 단계와 기준을 확인해야 하는지 정리한 기록이다.

## 배포 흐름을 보는 기준

CI/CD 파이프라인은 **코드 변경(커밋/PR)** 이 발생했을 때부터 **빌드 → 테스트 → 패키징(아티팩트 생성) → 배포**까지의 과정을 자동화해, 소프트웨어를 일관된 방식으로 전달하는 흐름이다.
...
CI/CD 파이프라인은 코드 변경을 자동으로 검증하고, 같은 산출물을 안전하게 배포 가능한 상태로 만드는 운영 흐름이다. 좋은 파이프라인은 배포를 빠르게 만드는 데서 끝나지 않고, 실패했을 때 되돌아갈 기준과 확인 지점을 같이 남긴다.
